%% Kinesthetic parameter exploration all clinical trial:
...
% This code generates all the components of fig S2A illustration.
...
% No hand trials to be removed
idx2remove = [];   % <- as provided
...
aMin = 1; aMax = max(AmplitudeData);
fMin = 1; fMax = max(FrequencyData);
...
hold off;
xlim([1 132]); ylim([1 132]); zlim([0 32]);
